...
@Component({
  selector: "ngx-select-v1",
  templateUrl: "./ng-select.component.html",
  styleUrls: ["./ng-select.component.scss"],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => NgxSelectV1Component),
      multi: true,
    },
  ],
...
export class NgxSelectV1Component
  implements OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor
{
  ...
  @ContentChildren(NgxOptionV1Component, { descendants: true })
  ngOptions: QueryList<NgxOptionV1Component>;
  ...
  selectTag() {
    ...
    if (isPromise(tag)) {
      tag
        .then((item) => this.select(this.itemsList.addItem(item)))
        .catch(() => {});
    } else if (tag) {
  ...
  showClear() {
    return (
      this.clearable && (this.hasValue || this.filterValue) && !this.isDisabled
    );
  }
  ...
  private _setItemsFromNgOptions() {
    const handleNgOptions = (options: QueryList<NgxOptionV1Component>) => {
